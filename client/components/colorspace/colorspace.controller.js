/*global angular, injectToThis, THREE*/
...
ColorspaceController.$name = 'ColorspaceController';

function ColorspaceController () {
	injectToThis(this.constructor).apply(this, arguments);

	var vm = this;

	vm.$scope.$watch('vm.pixelData', function (pixelData) {
		if( ! pixelData ) {
			return;
		}

		vm.debug('got pixel data', pixelData);

		if( vm.localStorageService.get('colorspace.alwaysDraw') ) {
			vm.drawColorspace();
		}
	});

	vm.alwaysDraw = vm.localStorageService.get('colorspace.alwaysDraw');
}

ColorspaceController.prototype.alwaysDrawToggle = function () {
	var vm = this;

	vm.localStorageService.set('colorspace.alwaysDraw', !! vm.alwaysDraw);
};

ColorspaceController.prototype.drawColorspace = function () {
	var vm = this;
	...
	colorspaceWorker.addEventListener('message', function (event) {
		vm.renderColorspace(event.data);
	}, false);
};

ColorspaceController.prototype.renderColorspace = function (uniqueColors) {
	var vm = this;
