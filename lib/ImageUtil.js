...
export async function getExiv2Info(path) {
	...
	return _.omitBy({
		exif: _.get(exiv2Data, 'Exif'),
		xmp: _.get(exiv2Data, 'Xmp'),
		iptc: _.get(exiv2Data, 'Iptc'),
	}, _.isUndefined)
}

export async function queueAnalysis(options) {
	debug('queueAnalysis', options)
	const params = {}
	//ela
	if( options.ela ) {
	...
	if( Object.keys(params).length < 1 ) {
		throw new HTTPError(400, 'no valid params found')
	}
	//job meta
