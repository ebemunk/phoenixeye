...
import DB from './DB'
import {getGMInfo, getExiv2Info} from './ImageUtil'
...
async function getImageMeta(path) {
	const [gmData, exiv2Data] = await Promise.all([
		getGMInfo(path),
		getExiv2Info(path)
	])
	return {
...
export async function handleJob(params, callback) {
	Promise.onPossiblyUnhandledRejection(err => {
		...
		console.log('fogg daatdd');
		callback(err)
	})
	log('handleJob', params)
	let image = await getImage(params.imageId)
	const path = await downloadImage(image)
	//get image metadata
	const {gmData, exiv2Data} = await getImageMeta(path)
	//run phoenix
	const output = await runCmd(params, path)
	//save metadata to db
	_.assign(image, {
		imagemagickQuality: _.get(output, 'imagick_estimate', null),
		hackerfactorQuality: _.get(output, 'hf_estimate', null),
		qtables: _.get(output, 'qtables', null),
		...gmData,
		...exiv2Data,
		metaComplete: true,
	})
	...
	const analyses = await Promise.map(analysesOutput, uploadAnalysis)
	//save analyses to db
	const db = await DB.get()
